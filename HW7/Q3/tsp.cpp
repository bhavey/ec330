...
struct moves {
    ...
    map<string,pair<int,int> > poss; //All the possible cities. Will get sorted at the end.
...
int main() {
    ...
    while (in >> entry) {
        if ((i+4)%4==0) { //Source
            ...
            Paths[city_names[j]].poss[cur_dest].first=atoi(entry.c_str());
        }
        i++;
    }
    ...
    for (int i=1; i<CITY_NUM; i++) {
        //First things first, fill the Paths.second pair values w. their #s of hits
...
//        vector<map<string,int> > stupidworkaround;
    ...
    for (int i=0; i<1000000; i++) {
        untouched="NO";
        for (map<string,pair<int,int> >::iterator j=Paths[cur_loc].poss.begin(); j!=Paths[cur_loc].poss.end(); j++) { //Find the next place to be!
            int poop=Paths[cur_loc].num;
            if (seen[j->first]==0) {
                if (untouched=="NO") {
                    untouched=j->first; //Just through in the first one
                    untprice=j->second.first;
                    printf("wat");
                }
                if (rand()%3<2) {//2/3 probability of taking it!
                    tmppair=j->second;
                    newprice=tmppair.first;
                    prev_loc=cur_loc;
                    cur_loc=j->first;
                    places_visited++;
                    break;
